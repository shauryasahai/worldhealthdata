...
class Map {
    ...
    drawMap(world) {
        ...
        // world is a topojson file. 
        // you will have to convert this to geojson 
        // (hint: you should have learned this in class!)
    ...
    updateHighlightClick(activeCountry) {
        ...
        let countryPath = d3.select('#'+activeCountry);
        let region = countryPath.attr("class");

        switch(region) {
            case 'asia': countryPath.classed('selected-country', true);
                         break;
            case 'africa': countryPath.classed('selected-country', true);
                           break;
            case 'europe': countryPath.classed('selected-country', true);
                           break;
            case 'americas': countryPath.classed('selected-country', true);
                             break;
           }
    ...
    clearHighlight() {
        ...
        for(var pop of this.populationData) {
            let countryID = pop['geo'].toUpperCase();
            let countryPath = d3.select('#'+countryID);
